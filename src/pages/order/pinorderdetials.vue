<template>
  <div class="contain">
    <div class="box">
      ...
      <!-- //0 待支付 1拼团中 2待发货 3已发货 4待取货 5待评价 6已评价 7已取消 8待退款 9已退款 10拒绝退款 -->
      <div class="cancel" v-if="state2 ==1">
        <span style="font-size:14px;margin-bottom:0.4rem;">拼团中</span>
        <div class="look" style="font-size:14px;color:#fff;" @click="shentui">申请退款</div>
      </div>
      <!-- <div class="wait" v-if="state2 == 1">拼团中</div> -->
      <!-- <div class="wait" v-if="state2 == 2">商家已接单，待发货</div> -->
      <div class="cancel" v-if="state2 ==2">
        <span style="font-size:14px;margin-bottom:0.4rem;">商家已接单，待发货</span>
        <div class="look" style="font-size:14px;color:#fff;" @click="shentui">申请退款</div>
      </div>
      <div class="cancel" v-if="state2 == 3">
      ...
      <div class="cancel" v-if="state2 == 4">
        <span style="font-size:14px;margin-bottom:0.4rem;">待取货</span>
        <div class="look" style="font-size:14px;color:#fff;" @click="shentui">申请退款</div>
      </div>
      <div class="cancel" v-if="state2 == 5">
      ...
      <div class="cancel" v-if="state2 == 6">
      ...
      <div class="cancel" v-if="state2 ==7">
      ...
      <div class="cancel" v-if="state2 == 8">
      ...
      <div class="cancel" v-if="state2 == 9">
    ...
    <div class="orders">
      ...
      <div class="user_coupon" v-if="state2 == 6">
        <span>取消时间</span>
        <div class="c_coupon">
      ...
      <div
        class="user_coupon"
        v-if="state2 == 1||state2 == 2||state2 == 3 || state2 == 4 || state2 == 5 ||state2 == 8 || state2 == 9"
      >
        <span>付款时间</span>
        <div class="c_coupon">
      ...
      <div
        class="user_coupon"
        v-if="state2 == 2||state2 == 3 || state2 == 4 || state2 == 5 ||  state2 == 7 || state2 == 8 || state2 == 9"
      >
        <span>拼团成功时间</span>
        <div class="c_coupon">
          <span>{{orders.groupsuccessdate}}</span>
        </div>
      </div>
      <!-- 0待付款 1待处理 2待发货 3待收货 4待评价 5已评价 6已取消 7待退款 8已退款 9拒绝退款 -->
      <div
        class="user_coupon"
        v-if=" state2 == 4 || state2 == 5 ||  state2 == 7 || state2 == 8 || state2 == 9"
      >
        <span>发货时间</span>
        <div class="c_coupon">
      ...
      <div
        class="user_coupon"
        v-if="state2 == 3 || state2 == 4 || state2 == 5 ||  state2 == 7 || state2 == 8 || state2 == 9"
      >
        <span>团长签收时间</span>
        <div class="c_coupon">
          <span>{{orders.qianshoudate}}</span>
        </div>
      </div>
      <div
        class="user_coupon"
      ...
      <div class="user_coupon" v-if=" state2 == 5 ||  state2 == 7 || state2 == 8 || state2 == 9">
        <span>评价时间</span>
        <div class="c_coupon">
          <span>{{orders.evaluateTime}}</span>
        </div>
      </div>
      <div class="user_coupon" v-if="  state2 == 7 || state2 == 8 || state2 == 9">
        <span>申请售后时间</span>
        <div class="c_coupon">
          <span>{{orders.applyRefundTime}}</span>
        </div>
      </div>
      <div class="user_coupon last" style="border-bottom: none;" v-if=" state2 == 8 || state2 == 9">
        <span>商家处理时间</span>
        <div class="c_coupon">
    ...
    <div class="sure" v-if="state2 == 3 ||state2 == 4" @click="shou">确认收货</div>
    <div class="sure" v-if="state2==0" @click="quxiao">取消订单</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    orderDetail(id) {
      ...
      //0待支付 1拼团中 2待发货 3已发货 4待取货 5待评价 6已评价 7已取消 8待退款 9已退款 10拒绝退款
    ...
    PayBywx(timeStamp, nonceStr, prepay_id, signType, paySign) {
      wx.requestPayment({
        ...
        success(res) {
          wx.navigateTo({
            url: "/pages/order/pay_success?id=" + 1
          });
          console.log("支付成功!");
        },
        fail(res) {
